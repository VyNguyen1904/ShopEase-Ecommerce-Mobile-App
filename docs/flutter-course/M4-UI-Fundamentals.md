...
class AppDrawer extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Drawer(
      // width defaults to min(304, screen-width - 56)
      child: ListView(
          ...
          DrawerHeader(
            ...
            child: Column(
              ...
              children: [
                ...
                const Text('Jane Doe', style: TextStyle(color: Colors.white, fontSize: 18)),
                Text('jane@shopease.com', style: TextStyle(color: Colors.white70, fontSize: 13)),
              ],
            ),
          ),
          // Navigation items
...
Material 3 introduces `NavigationDrawer` with built-in selected state highlighting:

```dart
class M3Drawer extends StatefulWidget {
  const M3Drawer({super.key});

  @override
  State<M3Drawer> createState() => _M3DrawerState();
}

class _M3DrawerState extends State<M3Drawer> {
    ...
    return NavigationDrawer(
      ...
      children: const [
        Padding(
          ...
          child: Text('ShopEase', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 20)),
        ),
...
### 17.1.3 BottomNavigationBar vs NavigationBar (Material 3)

Both serve the same purpose — bottom navigation — but `NavigationBar` is the Material 3 successor.

#### BottomNavigationBar (Material 2)

```dart
class BnbDemo extends StatefulWidget {
  const BnbDemo({super.key});
  @override
  State<BnbDemo> createState() => _BnbDemoState();
}
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      bottomNavigationBar: BottomNavigationBar(
        ...
        // type: fixed (all labels visible) vs shifting (labels only for selected)
...
#### NavigationBar (Material 3) — Preferred for new apps
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      bottomNavigationBar: NavigationBar(
        ...
        // indicatorColor highlights the selected icon background
        ...
        destinations: const [
          ...
          NavigationDestination(
            icon: Badge(          // Badge shows a notification count!
              label: Text('3'),
...
## 17.2 ListView Constructors

### Why ListView Exists

When your content exceeds the screen height, you need scrolling. `ListView` is Flutter's primary scrollable widget for a **linear list of items**. Understanding its four constructors is critical for both usability and performance.

### 17.2.1 `ListView()` — The Basic Constructor

Use this only for **short, fixed lists** (e.g., fewer than ~20 items).

```dart
/// All children are built immediately — not lazy!
ListView(
  ...
  children: [
    ...
    // ... all items created upfront in memory
  ],
)
```

### 17.2.2 `ListView.builder()` — Lazy Loading Hero ⭐

This is what you use for **any dynamic or long list**. Items are built only when they are about to become visible.

```dart
/// Simulated product list
final List<Map<String, String>> products = List.generate(
  1000,
  (i) => {'name': 'Product $i', 'price': '\$${(i * 9.99).toStringAsFixed(2)}'},
);
...
### 17.2.3 `ListView.separated()` — Built-in Dividers

Like `builder`, but accepts a `separatorBuilder` for inserting widgets between items (dividers, ads, etc.).

```dart
ListView.separated(
  itemCount: products.length,
  // separatorBuilder runs between every pair of items
  separatorBuilder: (context, index) {
    // You can vary the separator based on index!
    if (index % 5 == 4) {
      // Every 5th separator is an ad banner
      return Container(
        height: 80,
...
### 17.2.4 `ListView.custom()` — Full Control

For advanced use cases — uses a `SliverChildDelegate` directly.
...
ListView.custom(
  childrenDelegate: SliverChildBuilderDelegate(
    ...
    childCount: 50,
    // addAutomaticKeepAlives: keep off-screen items alive
    addAutomaticKeepAlives: false,
    // addRepaintBoundaries: isolate repaints
    addRepaintBoundaries: true,
  ),
)
```

### 17.2.5 Performance: Builder vs Children List

This is one of the most important distinctions in Flutter UI performance.
...
> 💡 **Pro Tip:** Set `itemExtent` when all list items have the same height. Flutter can then skip layout calculations for off-screen items, making scrolling even smoother on large lists.

#### Common Mistakes — ListView

- ❌ **Nesting `ListView` inside `Column`** — Column children need bounded heights. A ListView inside a Column will throw an "unbounded height" error. Fix: wrap the `ListView` in an `Expanded` widget.
- ❌ **Forgetting `itemCount` in `ListView.builder`** — Without it, the list scrolls infinitely. This will cause an infinite loop in your `itemBuilder`.
- ❌ **Using `ListView()` with hundreds of children** — Bad for performance. Switch to `ListView.builder`.
- ❌ **Not providing `padding: EdgeInsets.zero`** on the Drawer's ListView — Leads to ugly default top padding.

---

## 17.3 Card, ListTile, InkWell & GestureDetector
...
Card(
  ...
  shape: RoundedRectangleBorder(
    ...
    // You can add a border too:
    // side: BorderSide(color: Colors.purple, width: 1),
  ),

  // color: background color (defaults to theme's surface color)
  color: Colors.white,

  // shadowColor: customize the shadow color
  shadowColor: Colors.deepPurple.withOpacity(0.3),

  // margin: space outside the card
  margin: const EdgeInsets.symmetric(horizontal: 16, vertical: 8),

  // clipBehavior: clips content to the card's shape (important for images!)
  clipBehavior: Clip.antiAlias,

  child: Column(
    crossAxisAlignment: CrossAxisAlignment.start,
    children: [
      // Image fills the rounded top corners thanks to clipBehavior
      Image.network(
      ...
      const Padding(
        ...
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            Text('Nike Air Max', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
            SizedBox(height: 4),
            Text('\$129.99', style: TextStyle(color: Colors.deepPurple, fontSize: 16)),
            SizedBox(height: 8),
            Text('Classic design with maximum cushioning.', style: TextStyle(color: Colors.grey)),
          ],
        ),
      ),
    ],
...
```dart
ListTile(
  // leading: widget on the far left (icon, avatar, checkbox)
  ...
  // title: primary text
  title: const Text('Jane Doe', style: TextStyle(fontWeight: FontWeight.w600)),

  // subtitle: secondary text (shown below title, slightly smaller + greyed)
  subtitle: const Text('Premium Member since 2023'),

  // trailing: widget on the far right
  trailing: const Icon(Icons.chevron_right),

  // selected: highlights the tile with the theme's primary color
  selected: false,

  // selectedTileColor: background when selected
  selectedTileColor: Colors.deepPurple.withOpacity(0.1),

  // tileColor: background when NOT selected
  tileColor: Colors.white,

  // dense: reduces vertical height (good for compact lists)
  dense: false,

  // contentPadding: override internal padding
  contentPadding: const EdgeInsets.symmetric(horizontal: 16, vertical: 4),

  // enabled: greyed out when false
  enabled: true,
...
### 17.3.3 InkWell vs GestureDetector

When you want to make ANY widget tappable:

```dart
/// InkWell — Material ripple effect + gesture detection
/// Use this for Material Design apps
InkWell(
  onTap: () => print('Tapped!'),
  onDoubleTap: () => print('Double tapped!'),
  onLongPress: () => print('Long pressed!'),
  // borderRadius must match the parent's borderRadius
  borderRadius: BorderRadius.circular(12),
  // splashColor: color of the ripple
  splashColor: Colors.deepPurple.withOpacity(0.3),
  // highlightColor: color of the pressed state
...
GestureDetector(
  onTap: () => print('Tapped!'),
  onPanUpdate: (details) {
    // details.delta.dx and .dy give the drag direction
    print('Dragging: ${details.delta}');
  },
  onScaleUpdate: (details) {
    // details.scale: 1.0 = no scale, 2.0 = double size
    print('Scale: ${details.scale}');
  },
  onHorizontalDragEnd: (details) {
    if (details.primaryVelocity! > 0) print('Swiped right');
    if (details.primaryVelocity! < 0) print('Swiped left');
  },
...
### 17.3.4 Spacing Best Practices: SizedBox vs Padding vs margin

This is a common source of confusion. Here's the definitive guide:

```dart
Column(
  children: [
    // ── SizedBox ───────────────────────────────────────────────────────────
    // Use SizedBox for fixed whitespace BETWEEN siblings.
    // It's lightweight — literally just empty space.
    const Text('Item A'),
    ...
    // ── Padding ────────────────────────────────────────────────────────────
    // Use Padding to add space AROUND a child widget.
    Padding(
    ...
    // ── Container margin ───────────────────────────────────────────────────
    // Use margin on Container/Card for space OUTSIDE the widget boundary.
    // (Note: not all widgets have a margin property — only Container, Card, etc.)
    Container(
      margin: const EdgeInsets.all(12),   // space outside the container
      padding: const EdgeInsets.all(8),   // space inside the container
      color: Colors.grey.shade200,
      child: const Text('Padded container with margin'),
    ),
  ],
)
```
...
- ❌ **Using `Container` with `height`/`width` just for spacing** — Use `SizedBox` instead; it has less overhead.
- ❌ **Nesting unnecessary `Padding` widgets** — One `Padding(padding: EdgeInsets.all(X))` is enough; don't nest multiple.
- ❌ **Forgetting `clipBehavior: Clip.antiAlias` on Card** with child images — The image will overflow the rounded corners.
